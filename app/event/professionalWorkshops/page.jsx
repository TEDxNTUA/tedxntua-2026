...
const ITEM_SOCIAL_HOVER_COLOR = "#239d54";
    ...
    <div className={`${styles.socials} ${className}`}>
      {links.map((link) => (
        <SocialButton
          ...
          size={size}
          mode="blackgreen"
          hoverColor={ITEM_SOCIAL_HOVER_COLOR}
  ...
  return (
    <section className={styles.modalDescription}>
      <h3 className={styles.modalDescriptionName}>
        {formatUppercaseNoAccents("Description")}
      </h3>
...
function buildItemCard(item, index) {
  ...
  return {
    ...
    bios: [
      {
        name: item.name,
        text: item.personalDescription || item.personal || "",
      },
    ],
    socialLinks: getSocialLinks(item.socials),
  };
}

const itemCards = allProfWorkshops.map(buildItemCard).filter(Boolean);
    ...
    >
      ...
      >
        ...
        <div className={styles.modalContent}>
          <p className={styles.modalEyebrow}>Professional Workshop</p>
      ...
      <div className={styles.content}>
        <div className={styles.hero}>
          <h1 className={`${copixelDisplay.className} ${styles.title}`}>
            Meet our Professional Workshops
          </h1>
        </div>
